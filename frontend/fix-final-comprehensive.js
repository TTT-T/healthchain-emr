...
function fixAllErrors(content, filePath) {
  ...
  if (filePath.includes('dashboard/page.tsx')) {
    ...
    fixed = fixed.replace(/visit\.visit_date && visit\.visit_date\.startsWith\(today\)/g, "'visit_date' in visit && visit.visit_date && typeof visit.visit_date === 'string' && visit.visit_date.startsWith(today)");
    ...
    // Fix patient_name vs patientName
    fixed = fixed.replace(/queue\.patient_name/g, 'queue.patientName');
  }
  ...
  // Fix patient_name vs patientName in various files
  if (filePath.includes('queue-history/page.tsx') || filePath.includes('register-patient/page.tsx') || 
      filePath.includes('my-requests/page.tsx') || filePath.includes('notificationService.ts') || 
      filePath.includes('pushNotificationService.ts') || filePath.includes('emailTemplates.ts')) {
    fixed = fixed.replace(/\.patient_name\b/g, '.patientName');
  }
