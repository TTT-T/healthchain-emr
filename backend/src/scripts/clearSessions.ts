...
async function clearOldSessions() {
  try {
    ...
    const result = await databaseManager.query('DELETE FROM user_sessions WHERE expires_at < NOW()');
    console.log('✅ Cleared ' + result.rowCount + ' expired sessions');
    
    // Clear sessions with invalid user references
    const result2 = await databaseManager.query('DELETE FROM user_sessions WHERE user_id NOT IN (SELECT id FROM users)');
    console.log('✅ Cleared ' + result2.rowCount + ' orphaned sessions');
    
    // Clear all sessions for safety in development
    const result3 = await databaseManager.query('DELETE FROM user_sessions');
